...
export async function POST(req: Request) {
  ...
  console.log("UNV PeopleCount raw payload:", raw);
  ...
  const refRaw =
    typeof reference === "string" ? reference.split("/")[0]?.trim() : undefined;
  const refIp = refRaw ? extractIp(refRaw) ?? refRaw : undefined;
  const ip = refIp || getRequestIp(req, payload, raw);
  ...
  if (Array.isArray(list)) {
    const db = getDbPool();
    const cameraIp = extractIp(ip) ?? ip;
